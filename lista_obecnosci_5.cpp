...
public:
    ...
    virtual void drukuj() const {
        cout << imie << " " << nazwisko << " (" << dataUrodzenia << ", " << wzrost << "cm)";
    }
};
...
public:
    ...
    string serialize() const override {
        return getTyp() + ";" + indeks + ";" + imie + ";" + nazwisko + ";" + dataUrodzenia + ";" + to_string(wzrost);
    }
...
public:
    ...
    string serialize() const override {
        return getTyp() + ";" + to_string(id_pracownika) + ";" + imie + ";" + nazwisko + ";" + dataUrodzenia + ";" + to_string(wzrost);
    }
...
public:
    ...
    string serialize() const override {
        string wynik = "ListaObecnosci;" + nazwaListy + ";" + to_string(iloscNaLiscie);
        return wynik;
    }
...
private:
    ...
    void dodajOsobe() {
        ...
        if (typ == 1) {
            ...
            if (s->setNazwisko(nazw) && s->setImie(im) && s->setDataUrodzenia(data) && 
                s->setWzrost(wzr) && s->setIndeks(ind)) {
                nowaOsoba = s;
            } else {
                delete s;
            }
            ...
            int id;
            cout << "ID pracownika (wieksze od 0): "; cin >> id; cin.ignore();
            
            Pracownik* p = new Pracownik();
            if (p->setNazwisko(nazw) && p->setImie(im) && p->setDataUrodzenia(data) && 
                p->setWzrost(wzr) && p->setIdPracownika(id)) {
                nowaOsoba = p;
            } else {
                delete p;
            }
    ...
    void dodajListe() {
        if (iloscList >= maxList) { cout << "Brak miejsca na listy!\n"; return; }
        string nazwa;
    ...
    void ustawObecnosc() {
        ...
        if (!tablicaList[idxList].ustawObecnosc(nazw, ob)) cout << "Nie znaleziono osoby na liscie.\n";
    }
    ...
    void zmienDaneOsoby() {
        string nazw;
        cout << "Nazwisko osoby do edycji: "; getline(cin, nazw);
        int idxOs = znajdzOsobeWbazie(nazw);
        if (idxOs == -1) { cout << "Osoba nie znaleziona.\n"; return; }
        string nN; int nW;
